...
for (u in 1:8 ) {
	rand.bet[[u]] <- foreach(i=1:nrep, .combine=cbind) %dopar% {
	as.matrix(round(betweenness(g.rand[[u]][[i]], normalized=T), digits = 4 ))
}}
...
z.bet <- foreach(u=1:8) %dopar% {
	calc <- foreach(i=1:nrow(bet[[u]]), .combine=rbind) %dopar% {
		(bet[[u]][i] - mean(rand.bet[[u]][i,])) /  sd(rand.bet[[u]][i,])
	}
	calc
	}
...
P.bet <- foreach(u=1:8) %dopar% {
	calc <- foreach(i=1:nrow(bet[[u]]), .combine=rbind) %dopar% {
		sum(rand.bet[[u]][i,] > bet[[u]][i]) /  length(rand.bet[[u]][i,])
	}
	calc
	}
...
saveRDS(df1, file=sprintf("../Data/Centrality_rand_%s.rds", nrep))
write.table(df1, file=sprintf("../Output/Centrality_rand_%s.txt", nrep), sep="\t", quote=F, row.names=F)


pdf(sprintf('../Output/Counts_Bet.z_rand_%s.pdf', nrep), w= 30, h= 15)
g <- ggplot(df1) +
