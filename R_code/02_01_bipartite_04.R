...
# hunting_mode
head(spider.info)
#as.matrix(unique(spider.info$hunting_mode))
# PreyのOTU情報
...
mat <- read.table(sprintf('../Data/prey.id.prey.matrix.agg.%s.txt',i),header=T,sep ='\t',row.names=1)
...
tmp[tmp[,7]== "sit-and-wait","col"] <-'palevioletred1'
tmp[tmp[,7]=="web-weabing" ,"col"] <-  'darkseagreen'
tmp[tmp[,7]=="active-hunting","col"] <-　'rosybrown1'
tmp[tmp[,7]== "Attack" ,"col"] <-    'rosybrown4'   
tmp[tmp[,7]== "freeloader" ,"col"] <-'grey63'
...
tmp[tmp[,6]=='Blattodea',"col"] <-"#FBB4AE"
tmp[tmp[,6]=='Thysanoptera',"col"] <-"#B3CDE3"
tmp[tmp[,6]=='Psocoptera',"col"] <-"#CCEBC5"
tmp[tmp[,6]=='Odonata',"col"] <-"#DECBE4"
tmp[tmp[,6]=='Psocoptera',"col"] <-"#FED9A6"
tmp[tmp[,6]=='Dermaptera',"col"] <-"#FFFFCC"
tmp[tmp[,6]=='Strepsiptera',"col"] <-"#E5D8BD"
